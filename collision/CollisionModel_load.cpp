...
void idCollisionModelManagerLocal::LoadProcBSP( const char* name )
{
	...
	if( !src->IsLoaded() ) {
		idLib::Warning( "idCollisionModelManagerLocal::LoadProcBSP: couldn't load %s", filename.c_str() );
		delete src;
		return;
	}

	if( !src->ReadToken( &token ) || token.Icmp( PROC_FILE_ID ) ) {
		idLib::Warning( "idCollisionModelManagerLocal::LoadProcBSP: bad id '%s' instead of '%s'", token.c_str(), PROC_FILE_ID );
		delete src;
		return;
	}
...
cm_model_t* idCollisionModelManagerLocal::AllocModel()
{
	...
	model->numPolygons = model->polygonMemory = model->numBrushes = model->brushMemory = model->numNodes = model->numBrushRefs = model->numPolygonRefs = model->numInternalEdges =
		model->numSharpEdges = model->numRemovedPolys = model->numMergedPolys = model->usedMemory = 0;
...
void idCollisionModelManagerLocal::SetupTrmModelStructure()
{
	...
	if( !trmMaterial ) {
		idLib::FatalError( "_tracemodel material not found" );
	}
			...
			for( planeNum = startPlane, i = 0; i < b->numPlanes; i++, planeNum++ ) {
				...
				if( res == SIDE_BACK ) {
					if( cm_outList->numWindings >= MAX_WINDING_LIST ) {
						idLib::Warning( "idCollisionModelManagerLocal::ChopWindingWithBrush: primitive %d more than %d windings", list->primitiveNum, MAX_WINDING_LIST );
						return;
					}
...
cm_polygon_t* idCollisionModelManagerLocal::TryMergePolygons( cm_model_t* model, cm_polygon_t* p1, cm_polygon_t* p2 )
{
	...
	if( !keep1 ) {
		...
		GetEdge( model,
			model->vertices[model->edges[abs( edgeNum1 )].vertexNum[INT32_SIGNBITSET( edgeNum1 )]].p,
...
void idCollisionModelManagerLocal::CreatePolygon( cm_model_t* model, idFixedWinding* w, const idPlane& plane, const idMaterial* material, int primitiveNum )
{
	...
	if( numPolyEdges > CM_MAX_POLYGON_EDGES ) {
		idLib::Warning( "idCollisionModelManagerLocal::CreatePolygon: polygon has more than %d edges", numPolyEdges );
...
void idCollisionModelManagerLocal::PolygonFromWinding( cm_model_t* model, idFixedWinding* w, const idPlane& plane, const idMaterial* material, int primitiveNum )
{
	...
	if( w->IsHuge() ) {
		idLib::Warning( "idCollisionModelManagerLocal::PolygonFromWinding: model %s primitive %d is degenerate", model->name.c_str(), abs( primitiveNum ) );
		return;
	}
...
void idCollisionModelManagerLocal::FinishModel( cm_model_t* model )
{
	...
	model->usedMemory = model->numVertices * sizeof( cm_vertex_t ) + model->numEdges * sizeof( cm_edge_t ) + model->polygonMemory + model->brushMemory + model->numNodes * sizeof( cm_node_t ) +
						model->numPolygonRefs * sizeof( cm_polygonRef_t ) + model->numBrushRefs * sizeof( cm_brushRef_t );
}
...
cm_model_t* idCollisionModelManagerLocal::LoadRenderModel( const char* fileName )
{
	...
	if( ( extension.Icmp( "glb" ) != 0 ) && ( extension.Icmp( "gltf" ) != 0 ) && ( extension.Icmp( "ase" ) != 0 ) && ( extension.Icmp( "lwo" ) != 0 ) && ( extension.Icmp( "ma" ) != 0 ) &&
		( extension.Icmp( "dae" ) != 0 ) && ( extension.Icmp( "obj" ) != 0 ) ) {
		return NULL;
	}
	...
	ID_TIME_T sourceTimeStamp= FILE_NOT_FOUND_TIMESTAMP;// = renderModel->Timestamp();
...
{
	idLib::Printf( "%6i vertices (%i KB)\n", model->numVertices, ( model->numVertices * sizeof( cm_vertex_t ) ) >> 10 );
	idLib::Printf( "%6i edges (%i KB)\n", model->numEdges, ( model->numEdges * sizeof( cm_edge_t ) ) >> 10 );
	idLib::Printf( "%6i polygons (%i KB)\n", model->numPolygons, model->polygonMemory >> 10 );
	idLib::Printf( "%6i brushes (%i KB)\n", model->numBrushes, model->brushMemory >> 10 );
	idLib::Printf( "%6i nodes (%i KB)\n", model->numNodes, ( model->numNodes * sizeof( cm_node_t ) ) >> 10 );
	idLib::Printf( "%6i polygon refs (%i KB)\n", model->numPolygonRefs, ( model->numPolygonRefs * sizeof( cm_polygonRef_t ) ) >> 10 );
	idLib::Printf( "%6i brush refs (%i KB)\n", model->numBrushRefs, ( model->numBrushRefs * sizeof( cm_brushRef_t ) ) >> 10 );
	idLib::Printf( "%6i internal edges\n", model->numInternalEdges );
	idLib::Printf( "%6i sharp edges\n", model->numSharpEdges );
	idLib::Printf( "%6i contained polygons removed\n", model->numRemovedPolys );
	idLib::Printf( "%6i polygons merged\n", model->numMergedPolys );
	idLib::Printf( "%6i KB total memory used\n", model->usedMemory >> 10 );
}
...
void idCollisionModelManagerLocal::ModelInfo( cmHandle_t model )
{
	...
	if( model < 0 || model > MAX_SUBMODELS || model > maxModels ) {
		idLib::Printf( "idCollisionModelManagerLocal::ModelInfo: invalid model handle\n" );
		return;
	}
	if( !models[model] ) {
		idLib::Printf( "idCollisionModelManagerLocal::ModelInfo: invalid model\n" );
		return;
	}
...
void idCollisionModelManagerLocal::BuildModels( const idMapFile* mapFile, bool ignoreOldCollisionFile )
{
	...
	if( ignoreOldCollisionFile || !LoadCollisionModelFile( mapFile->GetName(), mapFile->GetGeometryCRC() ) ) {
		...
		for( i = 0; i < mapFile->GetNumEntities(); i++ ) {
			...
			if( numModels >= MAX_SUBMODELS ) {
				idLib::Error( "idCollisionModelManagerLocal::BuildModels: more than %d collision models", MAX_SUBMODELS );
				break;
			}
	...
	idLib::Printf( "collision data:\n" );
	idLib::Printf( "%6i models\n", numModels );
	PrintModelInfo( &model );
	idLib::Printf( "%.0f msec to load collision data.\n", timer.Milliseconds() );
}
...
void idCollisionModelManagerLocal::Preload( const char* mapName )
{
	...
	//	idLib::Printf( "Preloading collision models...\n" );
	...
	//	idLib::Printf( "%05d collision models preloaded ( or were already loaded ) in %5.1f seconds\n", numLoaded, ( end - start ) * 0.001 );
	//	idLib::Printf( "----------------------------------------\n" );
	//}
}
...
{
	if( mapFile == NULL ) {
		idLib::Error( "idCollisionModelManagerLocal::LoadMap: NULL mapFile" );
		return;
	}
	...
	if( loaded ) {
		if( mapName.Icmp( mapFile->GetName() ) == 0 ) {
			if( mapFile->GetFileTime() == mapFileTime ) {
				idLib::Printf( "Using loaded version\n" );
				return;
			}
			idLib::Printf( "Reloading modified map\n" );
		}
		FreeMap();
	}
...
const char* idCollisionModelManagerLocal::GetModelName( cmHandle_t model ) const
{
	if( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
		idLib::Printf( "idCollisionModelManagerLocal::GetModelBounds: invalid model handle\n" );
		return "";
	}
...
{
	if( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
		idLib::Printf( "idCollisionModelManagerLocal::GetModelContents: invalid model handle\n" );
		return false;
	}
...
{
	if( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
		idLib::Printf( "idCollisionModelManagerLocal::GetModelVertex: invalid model handle\n" );
		return false;
	}

	if( vertexNum < 0 || vertexNum >= models[model]->numVertices ) {
		idLib::Printf( "idCollisionModelManagerLocal::GetModelVertex: invalid vertex number\n" );
		return false;
	}
...
{
	if( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
		idLib::Printf( "idCollisionModelManagerLocal::GetModelEdge: invalid model handle\n" );
		return false;
	}

	edgeNum = abs( edgeNum );
	if( edgeNum >= models[model]->numEdges ) {
		idLib::Printf( "idCollisionModelManagerLocal::GetModelEdge: invalid edge number\n" );
		return false;
	}
...
bool idCollisionModelManagerLocal::GetModelPolygon( cmHandle_t model, int polygonNum, idFixedWinding& winding ) const
{
	...
	if( model < 0 || model > MAX_SUBMODELS || model >= numModels || !models[model] ) {
		idLib::Printf( "idCollisionModelManagerLocal::GetModelPolygon: invalid model handle\n" );
		return false;
	}
...
cmHandle_t idCollisionModelManagerLocal::LoadModel( const char* modelName, const bool precache )
{
	...
	if( numModels >= MAX_SUBMODELS ) {
		idLib::Error( "idCollisionModelManagerLocal::LoadModel: no free slots\n" );
		return 0;
	}
	...
	if( LoadCollisionModelFile( modelName, 0 ) ) {
		...
		if( handle >= 0 && handle < numModels ) {
			...
			idLib::Warning( "idCollisionModelManagerLocal::LoadModel: collision file for '%s' contains different model", modelName );
		}
	}
...
bool idCollisionModelManagerLocal::TrmFromModel( const cm_model_t* model, idTraceModel& trm )
{
	...
	if( model->numVertices > MAX_TRACEMODEL_VERTS ) {
		idLib::Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s has too many vertices.\n", model->name.c_str() );
		PrintModelInfo( model );
		return false;
	}
	...
	if( model->numEdges > MAX_TRACEMODEL_EDGES + 1 ) {
		idLib::Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s has too many edges.\n", model->name.c_str() );
		PrintModelInfo( model );
		return false;
	}
	...
	if( !TrmFromModel_r( trm, model->node ) ) {
		idLib::Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s has too many polygons.\n", model->name.c_str() );
		PrintModelInfo( model );
		return false;
	}
	...
	for( i = 1; i <= trm.numEdges; i++ ) {
		if( numEdgeUsers[i] != 2 ) {
			idLib::Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s has dangling edges, the model has to be an enclosed hull.\n", model->name.c_str() );
			PrintModelInfo( model );
			return false;
		}
	}
...
bool idCollisionModelManagerLocal::TrmFromModel( const char* modelName, idTraceModel& trm )
{
	...
	if( !handle ) {
		idLib::Printf( "idCollisionModelManagerLocal::TrmFromModel: model %s not found.\n", modelName );
		return false;
	}
