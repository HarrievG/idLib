...
void idCollisionModelManagerLocal::TranslateTrmEdgeThroughPolygon( cm_traceWork_t* tw, cm_polygon_t* poly, cm_trmEdge_t* trmEdge )
{
	...
	for( i = 0; i < poly->numEdges; i++ ) {
		...
		epsPl.FromLine( tw->model->vertices[edge->vertexNum[0]].p + edge->normal * CM_CLIP_EPSILON, tw->model->vertices[edge->vertexNum[1]].p + edge->normal * CM_CLIP_EPSILON );
...
bool idCollisionModelManagerLocal::TranslateTrmThroughPolygon( cm_traceWork_t* tw, cm_polygon_t* p )
{
	...
	if( tw->pointTrace ) {
		...
		for( i = 0; i < p->numEdges; i++ ) {
			...
			tw->polygonEdgePlueckerCache[i].FromLine( tw->model->vertices[e->vertexNum[0]].p, tw->model->vertices[e->vertexNum[1]].p );
...
void idCollisionModelManagerLocal::Translation(
	trace_t* results, const idVec3& start, const idVec3& end, const idTraceModel* trm, const idMat3& trmAxis, int contentMask, cmHandle_t model, const idVec3& modelOrigin, const idMat3& modelAxis )
{
	int				 i, j;
	float			 dist;
	...
	if( model < 0 || model > MAX_SUBMODELS || model > idCollisionModelManagerLocal::maxModels ) {
		idLib::Printf( "idCollisionModelManagerLocal::Translation: invalid model handle\n" );
		return;
	}
	if( !idCollisionModelManagerLocal::models[model] ) {
		idLib::Printf( "idCollisionModelManagerLocal::Translation: invalid model\n" );
		return;
	}
	...
	if( tw.dir.LengthSqr() > Square( CM_MAX_TRACE_DIST ) ) {
		...
		//}
		idLib::Printf( "idCollisionModelManagerLocal::Translation: huge translation\n" );
		return;
	}
