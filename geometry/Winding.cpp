...
int idWinding::Split( const idPlane &plane, const float epsilon, idWinding **front, idWinding **back ) const {
	...
	sides = (byte *) alloca( (numPoints+4) * sizeof( byte ) );
	...
	*back = b = new idWinding(maxpts);
	...
	if ( f->numPoints > maxpts || b->numPoints > maxpts ) {
		fprintf(stderr, "idWinding::Split: points exceeded estimate.\n");
	}
...
bool idWinding::Check( bool print ) const {
	...
	if ( numPoints < 3 ) {
		if ( print ) {
			fprintf(stderr, "idWinding::Check: only %i points.", numPoints );
		}
		return false;
	}
	...
	if ( area < 1.0f ) {
		if ( print ) {
			fprintf(stderr, "idWinding::Check: tiny area: %f", area );
		}
		return false;
	}
	...
	for ( i = 0; i < numPoints; i++ ) {
		...
		for ( j = 0; j < 3; j++ ) {
			if ( p1[j] >= MAX_WORLD_COORD || p1[j] <= MIN_WORLD_COORD ) {
				if ( print ) {
					fprintf(stderr, "idWinding::Check: point %d outside world %c-axis: %f", i, 'X'+j, p1[j] );
				}
				return false;
			}
		}
		...
		if ( d < -ON_EPSILON || d > ON_EPSILON ) {
			if ( print ) {
				fprintf(stderr, "idWinding::Check: point %d off plane.", i );
			}
			return false;
		}
		...
		if ( dir.Length() < ON_EPSILON) {
			if ( print ) {
				fprintf(stderr, "idWinding::Check: edge %d is degenerate.", i );
			}
			return false;
		}
		...
		for ( j = 0; j < numPoints; j++ ) {
			...
			if ( d > edgedist ) {
				if ( print ) {
					fprintf(stderr, "idWinding::Check: non-convex." );
				}
				return false;
			}
		}
...
void idWinding::RemovePoint( int point ) {
	if ( point < 0 || point >= numPoints ) {
		fprintf(stderr, "idWinding::removePoint: point out of range\n" );
		return;
	}
...
void idWinding::InsertPoint( const idVec3 &point, int spot ) {
	...
	if ( spot > numPoints ) {
		fprintf(stderr, "idWinding::insertPoint: spot > numPoints\n" );
		return;
	}

	if ( spot < 0 ) {
		fprintf(stderr, "idWinding::insertPoint: spot < 0\n" );
		return;
	}
...
bool idFixedWinding::ReAllocate( int n, bool keep ) {
	...
	if ( n > MAX_POINTS_ON_WINDING ) {
		fprintf(stderr, "WARNING: idFixedWinding -> MAX_POINTS_ON_WINDING overflowed\n");
		return false;
	}
	return true;
}
