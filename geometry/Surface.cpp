...
int idSurface::Split( const idPlane &plane, const float epsilon, idSurface **front, idSurface **back, int *frontOnPlaneEdges, int *backOnPlaneEdges ) const {
	...
	dists = (float *) alloca( verts.Num() * sizeof( float ) );
	sides = (byte *) alloca( verts.Num() * sizeof( byte ) );
	...
	*back = surface[1] = new idSurface();
	...
	edgeSplitVertex = (int *) alloca( edges.Num() * sizeof( int ) );
	...
	vertexRemap[0] = (int *) alloca( verts.Num() * sizeof( int ) );
	memset( vertexRemap[0], -1, verts.Num() * sizeof( int ) );
	vertexRemap[1] = (int *) alloca( verts.Num() * sizeof( int ) );
	memset( vertexRemap[1], -1, verts.Num() * sizeof( int ) );

	vertexCopyIndex[0] = (int *) alloca( ( numEdgeSplitVertexes + verts.Num() ) * sizeof( int ) );
	vertexCopyIndex[1] = (int *) alloca( ( numEdgeSplitVertexes + verts.Num() ) * sizeof( int ) );
	...
	onPlaneEdges[0] = (int *) alloca( ( maxOnPlaneEdges + 1 ) * sizeof( int ) );
	onPlaneEdges[1] = (int *) alloca( ( maxOnPlaneEdges + 1 ) * sizeof( int ) );
...
bool idSurface::ClipInPlace( const idPlane &plane, const float epsilon, const bool keepOn ) {
	...
	// allocate indexes to construct the triangle indexes for the front and back surface
	vertexRemap = (int *) alloca( verts.Num() * sizeof( int ) );
	memset( vertexRemap, -1, verts.Num() * sizeof( int ) );

	vertexCopyIndex = (int *) alloca( ( numEdgeSplitVertexes + verts.Num() ) * sizeof( int ) );
...
bool idSurface::IsConnected( void ) const {
	...
	islandNum = (int *) alloca( numTris * sizeof( int ) );
	memset( islandNum, -1, numTris * sizeof( int ) );
	queue = (int *) alloca( numTris * sizeof( int ) );
...
bool idSurface::RayIntersection( const idVec3 &start, const idVec3 &dir, float &scale, bool backFaceCull ) const {
	...
	sidedness = (byte *)alloca( edges.Num() * sizeof(byte) );
...
void idSurface::GenerateEdgeIndexes( void ) {
	...
	vertexEdges = (int *) alloca( verts.Num() * sizeof( int ) );
	memset( vertexEdges, -1, verts.Num() * sizeof( int ) );
	edgeChain = (int *) alloca( indexes.Num() * sizeof( int ) );
