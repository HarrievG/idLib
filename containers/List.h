...
inline type *idListNewElementA( argType arg  ) {
	return new type(arg);
}
...
inline bool idList<_type_ >::RemoveIndexFast( int index ) {
...
inline void idList< type >::SortWithTemplate( const idSort< type > & sort ) {
	if ( list == NULL ) {
		return;
	}
	sort.Sort( Ptr(), Num() );
