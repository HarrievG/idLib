...
void idMapBrushSide::ConvertToValve220Format( const idMat4& entityTransform, idStrList& textureCollections )
{
	...
	idImage*		  image = NULL;//material->GetEditorImage();
	if( image != NULL ) {
	...
	if( SDL_isnan( texScale[0] ) ) {
		texScale[0] = 0.5f;
	}

	if( SDL_isnan( texScale[1] ) ) {
		texScale[1] = 0.5f;
	}

	if( texMat[0][0] < idMath::FLT_EPSILON ) {
		texValve[0] = -texValve[0];
		texScale[0] = -texScale[0];
	}

	if( texMat[1][0] < idMath::FLT_EPSILON ) {
...
bool idMapPatch::Write( idFile* fp, int primitiveNum, const idVec3& origin ) const
{
	...
	for( i = 0; i < GetWidth(); i++ ) {
		...
		for( j = 0; j < GetHeight(); j++ ) {
			...
			st = v->GetTexCoord();
			fp->WriteFloatString( " ( %f %f %f %f %f )", v->xyz[0] + origin[0], v->xyz[1] + origin[1], v->xyz[2] + origin[2], st[0], st[1] );
		}
		fp->WriteFloatString( " )\n" );
	}
	fp->WriteFloatString( "  )\n }\n}\n" );
...
void idMapBrush::SetPlanePointsFromWindings( const idVec3& origin, int entityNum, int primitiveNum )
{
	...
	for( int i = 0; i < GetNumSides(); i++ ) {
		...
		if( !w.GetNumPoints() ) {
			idLib::Printf( "Entity %i, Brush %i: base winding has no points\n", entityNum, primitiveNum );
			break;
		}
		...
		for( int j = 0; j < GetNumSides() && w.GetNumPoints(); j++ ) {
			...
			if( !w.ClipInPlace( -planes[j], 0 ) ) {
				idLib::Printf( "Entity %i, Brush %i: no intersection with other brush plane\n", entityNum, primitiveNum );
				break;
			}
		}
		...
		if( entities[0]->epairs.GetBool( "forceEntityNames" ) ) {
			for( i = 1; i < entities.Num(); i++ ) {
				...
				if( !mapEnt->epairs.FindKey( "name" ) ) {
					...
					//remove va use
					char fullName[64];
					idStr::snPrintf( fullName, 64, "%s&d", mapEnt->epairs.GetString( "classname", "forcedName" ),i );
					mapEnt->epairs.Set( "name", fullName );
...
bool idMapFile::Write( const char* fileName, const char* ext, bool fromBasePath )
{
	...
	if( fromBasePath ) {
		...
		fp = fileSystem->OpenExplicitFileWrite( qpath );
	}

	if( !fp ) {
		idLib::Warning( "Couldn't open %s\n", qpath.c_str() );
		return false;
	}
