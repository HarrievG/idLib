...
public:
	...
	int			UTF8Length();
	uint32_t		UTF8Char( int & idx );
	static int				UTF8Length( const byte * s );
	static  uint32_t UTF8Char( const char * s, int & idx );
	...
	static  bool	IsValidUTF8( const char * s, const int maxLen, utf8Encoding_t & encoding ) { return IsValidUTF8( ( const uint8_t* )s, maxLen, encoding ); }
	static  bool	IsValidUTF8( const uint8_t * s, const int maxLen );
	...
	static void *		ColorForIndex( int i );
	...
	static void			ShowMemoryUsage_f( const void *args );
...
========================
*/
inline uint32_t idStr::UTF8Char( int & idx ) {
...
========================
*/
inline void idStr::ConvertToUTF8() {
	idStr temp( *this );
	Clear();
...
========================
*/
inline uint32_t idStr::UTF8Char( const char * s, int & idx ) {
...
========================
*/
inline bool idStr::IsValidUTF8( const uint8_t * s, const int maxLen ) {
...
========================
*/
inline void idStr::CopyRange( const char *text, int start, int end ) {
	int l = end - start;
	if ( l < 0 ) {
		l = 0;
	}
