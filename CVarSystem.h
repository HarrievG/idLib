...
// Argument completion function type
...
// Arg completion for boolean is implemented in CmdSystem, so we need to forward declare or handle it.
// However, idCVar inline constructors use idCmdSystem::ArgCompletion_Boolean.
// We can't include CmdSystem.h here because it might circle back.
// Ideally, we move the inline constructors to .cpp or include CmdSystem.h (if guarded).
// For now, I will include CmdSystem.h only if it's safe or remove the default arg from inline.
// But the inline code uses idCmdSystem::ArgCompletion_Boolean.
// I will just declare it here as extern.
						  ...
						  argCompletion_t valueCompletion )
{
	...
	{
		valueCompletion = idCmdSystem_Static::ArgCompletion_Boolean;
	}
