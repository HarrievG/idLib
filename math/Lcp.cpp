...
void idLCP_Square::RemoveClamped( int r ) {
	...
	y0 = (float *) alloca( numClamped * sizeof( float ) );
	z0 = (float *) alloca( numClamped * sizeof( float ) );
	y1 = (float *) alloca( numClamped * sizeof( float ) );
	z1 = (float *) alloca( numClamped * sizeof( float ) );
	...
	for ( i = r; i < numClamped; i++ ) {
		...
		if ( diag == 0.0f ) {
			fprintf(stderr, "idLCP_Symmetric::RemoveClamped: updating factorization failed\n" );
			return;
		}
...
ID_MAYBE_INLINE void idLCP_Square::CalcForceDelta(int d, float dir) {
	...
	ptr = (float *) alloca( numClamped * sizeof( float ) );
	for ( i = 0; i < numClamped; i++ ) {
		ptr[i] = rowPtrs[i][d];
	}
...
bool idLCP_Square::Solve( const idMatX &o_m, idVecX &o_x, const idVecX &o_b, const idVecX &o_lo, const idVecX &o_hi, const int *o_boxIndex ) {
	...
	if ( o_boxIndex ) {
		boxIndex = (int *)alloca( o_x.GetSize() * sizeof( int ) );
	...
	if ( numUnbounded ) {
		...
		if ( !FactorClamped() ) {
			fprintf(stderr, "idLCP_Square::Solve: unbounded factorization failed\n" );
			return false;
		}
	...
	for ( i = numUnbounded; i < m.GetNumRows(); i++ ) {
		...
		for ( n = 0; n < maxIterations; n++ ) {
			...
			if ( maxStep <= 0.0f ) {
				...
				numIgnored++;
#else
				static char failed_buf[128];
				snprintf(failed_buf, sizeof(failed_buf), "invalid step size %.4f", maxStep );
				failed = failed_buf;
#endif
				break;
			}
		...
		if ( n >= maxIterations ) {
			static char failed_buf[128];
			snprintf(failed_buf, sizeof(failed_buf), "max iterations %d", maxIterations );
			failed = failed_buf;
			break;
		}
...
#ifdef IGNORE_UNSATISFIABLE_VARIABLES
	if ( numIgnored ) {
		fprintf(stderr, "idLCP_Symmetric::Solve: %d of %d bounded variables ignored\n", numIgnored, m.GetNumRows() - numUnbounded );
	}
#endif

	// if failed clear remaining forces
	if ( failed ) {
		fprintf(stderr, "idLCP_Square::Solve: %s (%d of %d bounded variables ignored)\n", failed, m.GetNumRows() - i, m.GetNumRows() - numUnbounded );
...
void idLCP_Symmetric::AddClamped( int r, bool useSolveCache ) {
	...
	if ( useSolveCache ) {
		...
		float *v = (float *) alloca( numClamped * sizeof( float ) );
	...
	if ( d == 0.0f ) {
		fprintf(stderr, "idLCP_Symmetric::AddClamped: updating factorization failed\n" );
		numClamped++;
		return;
	}
...
void idLCP_Symmetric::RemoveClamped( int r ) {
	...
	addSub = (float *) alloca( numClamped * sizeof( float ) );
	...
	if ( r == 0 ) {
		...
		v = (float *) alloca( numClamped * sizeof( float ) );
	...
	v1 = (float *) alloca( numClamped * sizeof( float ) );
	v2 = (float *) alloca( numClamped * sizeof( float ) );
...
bool idLCP_Symmetric::Solve( const idMatX &o_m, idVecX &o_x, const idVecX &o_b, const idVecX &o_lo, const idVecX &o_hi, const int *o_boxIndex ) {
	...
	if ( numUnbounded ) {
		...
		// factor and solve for unbounded variables
		if ( !FactorClamped() ) {
			fprintf(stderr, "idLCP_Symmetric::Solve: unbounded factorization failed\n" );
			return false;
		}
		SolveClamped( f, b.ToFloatPtr() );
	...
	// if failed clear remaining forces
	if ( failed ) {
		fprintf(stderr, "idLCP_Symmetric::Solve: %s (%d of %d bounded variables ignored)\n", failed, m.GetNumRows() - i, m.GetNumRows() - numUnbounded );
		for ( j = i; j < m.GetNumRows(); j++ ) {
			f[j] = 0.0f;
		}
	}
