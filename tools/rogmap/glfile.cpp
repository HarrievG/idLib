	...
	if( node->planenum == PLANENUM_LEAF ) {
		// if( !node->opaque )
		{
			// if( node->area != -1 )
			{
				group = &groups.Alloc();
				if( node->opaque ) {
					sprintf( group->name, "area%i_leaf_opaque.%i", node->area, node->nodeNumber );
				} else {
					sprintf( group->name, "area%i_leaf.%i", node->area, node->nodeNumber );
				}
...
void OutputSplitPlane( const node_t* node, idList<OBJGroup>& groups )
{
	...
	if( node->planenum != PLANENUM_LEAF ) {
        ...
        sprintf( group.name, "splitplane.%i_%i", node->nodeNumber, planeCounter );
...
void OutputAreaPortalTriangles( const node_t* node, idList<OBJGroup>& groups )
{
	...
	if( node->planenum == PLANENUM_LEAF && node->areaPortalTris ) {
		OBJGroup& group = groups.Alloc();
		group.name.IsColor(); // Dummy call
        sprintf( group.name, "areaPortalTris.%i", node->nodeNumber );
...
void CollectAreaPortals( idList<OBJGroup>& groups )
{
	...
	for( i = 0; i < interAreaPortals.Num(); i++ ) {
		...
		OBJGroup& group = groups.Alloc();
		group.name.IsColor(); // Dummy call
        sprintf( group.name, "interAreaPortal.%i", i );
...
void WriteGLViewBSP( tree_t* tree, const char* source, int entityNum, bool force )
{
	...
	idStr path;
	sprintf( path, "%s_BSP_%s_%i.obj", dmapGlobals.mapFileBase, source, entityNum );
...
void WriteGLViewFacelist( bspFace_t* list, const char* source, bool force )
{
	...
	idStr path;
	sprintf( path, "%s_BSP_%s_%i.obj", dmapGlobals.mapFileBase, source, dmapGlobals.entityNum );
	idFileLocal		objFile( fileSystem->OpenFileWrite( path, "fs_basepath" ) );
