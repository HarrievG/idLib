...
void PrintTree_r( node_t* node, int depth )
{
	...
	if( node->planenum == PLANENUM_LEAF ) {
		...
		*/
		...
		idLib::Printf( "leaf %i", node->nodeNumber );
		if( node->area >= 0 ) {
			idLib::Printf( " area %i", node->area );
		}
		if( node->opaque ) {
			idLib::Printf( " opaque" );
		}
		if( node->occupied ) {
			idLib::Printf( " occupied" );
		}
		idLib::Printf( "\n" );
		return;
	}
	...
	idPlane& plane = dmapGlobals.mapPlanes[node->planenum];
	idLib::Printf( "#%i plane = %i (%5.2f %5.2f %5.2f %5.2f)\n", node->nodeNumber, node->planenum, plane[0], plane[1], plane[2], plane[3] );
...
int SelectSplitPlaneNum( node_t* node, bspFace_t* list )
{
	...
	auto scorePlane = [&]( bspFace_t* candidate, bool preferWallsOnly ) -> int
	{
		...
		// Area bonus (larger idLib:: get higher score)
