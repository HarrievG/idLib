...
/*
  ...
  new vertexes will be created when different idLib::
  cause a fix
...
void HashTriangles( optimizeGroup_t* groupList )
{
	...
	for( group = groupList; group; group = group->nextGroup ) {
		// don't create tjunctions against discrete idLib:: (blood decals, etc)
...
static mapTri_t* FixTriangleAgainstHashVert( const mapTri_t* a, const hashVert_t* hv )
{
	...
	for( i = 0; i < 3; i++ ) {
		...
		split.SetTexCoord( v1ST.x + frac * ( v2ST.x - v1ST.x ), v1ST.y + frac * ( v2ST.y - v1ST.y ) );
...
void FixAreaGroupsTjunctions( optimizeGroup_t* groupList )
{
	...
	idLib::Printf( "----- FixAreaGroupsTjunctions -----\n" );
	idLib::Printf( "%6i triangles in\n", startCount );

	HashTriangles( groupList );

	for( group = groupList; group; group = group->nextGroup ) {
		// don't touch discrete idLib::
//		if( group->material != NULL && group->material->IsDiscrete() ) {
		//	continue;
	//	}
...
void FixGlobalTjunctions( uEntity_t* e )
{
	...
	idLib::Printf( "----- FixGlobalTjunctions -----\n" );
