...
bool ProcessModels()
{
	...
	//idLib::RogmapPacifierCompileProgressTotal( dmapGlobals.numEntities );
	...
	for( dmapGlobals.entityNum = 0; dmapGlobals.entityNum < dmapGlobals.numEntities; dmapGlobals.entityNum++) {
		...
		if( dmapGlobals.entityNum == 0 ) {
			idLib::RogmapPacifierInfo( "Current entity   : worldspawn" );
		} else {
			idLib::RogmapPacifierInfo( "Current entity   : %s", entity->mapEntity->epairs.GetString( "name" ) );
		}
...
void RogmapHelp()
{
	idLib::Printf( "Usage: rogmap [options] mapfile\n"
					"Options:\n"
...
void Rogmap( const idCmdArgs& args )
{
	...
	idLib::RogmapPacifierFilename( passedName, "Compiling BSP .proc" );
    ...
    char buf[1024];
    sprintf(buf, "generated/%s.bproc", dmapGlobals.mapFileBase);
	fileSystem->RemoveFile( buf );

	// delete any old generated binary cm files
	sprintf(buf, "generated/%s.bcm", dmapGlobals.mapFileBase);
	fileSystem->RemoveFile( buf );
	...
	idLib::RogmapPacifierInfo( "%5.0f seconds for dmap\n", ( end - start ) * 0.001f );
	...
	if( !leaked ) {
		if( !noCM ) {
			...
			idLib::RogmapPacifierFilename( passedName, "Generating .cm collision map" );
			...
			idLib::RogmapPacifierInfo( "%5.0f seconds to create collision map\n", ( end - start ) * 0.001f );
		}
		...
		idLib::RogmapPacifierFilename( passedName, "Done" );
	} else {
		idLib::RogmapPacifierFilename( passedName, "Failed due to errors. Quit program." );
	}
...
{
	idLib::ClearWarnings( "running rogmap" );
