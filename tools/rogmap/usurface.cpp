...
void PutPrimitivesInAreas( uEntity_t* e )
{
	...
	idLib::Printf( "----- PutPrimitivesInAreas -----\n" );
	...
	if( dmapGlobals.entityNum == 0 ) {
		...
		for( int eNum = 1; eNum < dmapGlobals.numEntities; eNum++ ) {
			...
			if( inlineAll ) {
				...
				if( ( extension.Icmp( GLTF_GLB_EXT ) != 0 ) && ( extension.Icmp( GLTF_EXT ) != 0 ) && ( extension.Icmp( "lwo" ) != 0 ) && ( extension.Icmp( "ase" ) != 0 ) &&
					( extension.Icmp( "ma" ) != 0 ) && ( extension.Icmp( "obj" ) != 0 ) ) {
...
void FilterMeshesIntoTree( uEntity_t* e )
{
	...
	idLib::Printf( "----- FilterMeshesIntoTree -----\n" );
	...
	idLib::Printf( "%5i total BSP triangles\n", c_unique );
	idLib::Printf( "%5i cluster references\n", c_clusters );
}
// RB end
...
static void CarveGroupsByLight( uEntity_t* e, mapLight_t* light )
{
	...
	for( i = 0; i < e->numAreas; i++ ) {
		...
		for( group = area->groups; group; group = nextGroup ) {
			...
			//if( ( light->def.lightShader->IsFogLight() && !group->material->ReceivesFog() ) || ( !light->def.lightShader->IsFogLight() && !group->material->ReceivesLighting() ) ||
			...
			if( group->numGroupLights == MAX_GROUP_LIGHTS ) {
				idLib::Error( "MAX_GROUP_LIGHTS around %f %f %f", group->triList->v[0].xyz[0], group->triList->v[0].xyz[1], group->triList->v[0].xyz[2] );
			}

			// if the group doesn't face the light,
			// it won't get carved at all
			if( !light->def.lightShader->LightEffectsBackSides() && !group->material->ReceivesLightingOnBackSides() &&
...
void Prelight( uEntity_t* e )
{
	...
	if( !dmapGlobals.noMerge ) {
		idLib::Printf( "----- BuildLightShadows -----\n" );
		start = SDL_GetTicks();
		...
		idLib::Printf( "%5.1f seconds for BuildLightShadows\n", ( end - start ) / 1000.0 );
	}

	if( !dmapGlobals.noLightCarve ) {
		idLib::Printf( "----- CarveGroupsByLight -----\n" );
		start = SDL_GetTicks();
		...
		idLib::Printf( "%5.1f seconds for CarveGroupsByLight\n", ( end - start ) / 1000.0 );
	}
}
