...
static void AddInteriorEdges( optIsland_t* island )
{
	...
	idLib::Printf( "%6i tested segments\n", numLengths );
	idLib::Printf( "%6i added interior edges\n", c_addedEdges );
...
static void RemoveIfColinear( optVertex_t* ov, optIsland_t* island )
{
	...
	if( !e2 ) {
		...
		idLib::Printf( "WARNING: vertex with only one edge\n" );
		return;
	}
...
static void CombineColinearEdges( optIsland_t* island )
{
	...
	idLib::Printf( "%6i original exterior edges\n", c_edges );
	...
	idLib::Printf( "%6i optimized exterior edges\n", c_edges );
}
...
static void LinkTriToEdge( optTri_t* optTri, optEdge_t* edge )
{
	if( ( edge->v1 == optTri->v[0] && edge->v2 == optTri->v[1] ) || ( edge->v1 == optTri->v[1] && edge->v2 == optTri->v[2] ) || ( edge->v1 == optTri->v[2] && edge->v2 == optTri->v[0] ) ) {
		if( edge->backTri ) {
			idLib::Printf( "Warning: LinkTriToEdge: already in use\n" );
			return;
		}
		edge->backTri = optTri;
		return;
...
static void CreateOptTri( optVertex_t* first, optEdge_t* e1, optEdge_t* e2, optIsland_t* island )
{
	...
	if( !opposite ) {
		idLib::Printf( "Warning: BuildOptTriangles: couldn't locate opposite\n" );
		return;
	}
...
static void RegenerateTriangles( optIsland_t* island )
{
	...
	for( optTri = island->tris; optTri; optTri = optTri->next ) {
		...
		if( plane.Normal() * dmapGlobals.mapPlanes[island->group->planeNum].Normal() <= 0 ) {
			...
			idLib::Printf( "WARNING: backwards triangle generated!\n" );
			// discard it
			FreeTri( tri );
			continue;
		}
...
static void RemoveInteriorEdges( optIsland_t* island )
{
	...
	idLib::Printf( "%6i original interior edges\n", c_interiorEdges );
	idLib::Printf( "%6i original exterior edges\n", c_exteriorEdges );
}
...
static void				AddOriginalTriangle( optVertex_t* v[3] )
{
	...
	if( !IsTriangleValid( v[0], v[1], v[2] ) ) {
		idLib::Printf( "WARNING: backwards triangle in input!\n" );
		return;
	}
...
void SplitOriginalEdgesAtCrossings( optimizeGroup_t* opt )
{
	...
	idLib::Printf( "%6i original edges\n", numOriginalEdges );
	idLib::Printf( "%6i edges after splits\n", numOptEdges );
	idLib::Printf( "%6i original vertexes\n", numOriginalVerts );
	idLib::Printf( "%6i vertexes after splits\n", numOptVerts );
}
...
static void CullUnusedVerts( optIsland_t* island )
{
	...
	idLib::Printf( "%6i verts kept\n", c_keep );
	idLib::Printf( "%6i verts freed\n", c_free );
}
...
void OptimizeGroupList( optimizeGroup_t* groupList )
{
	...
	idLib::Printf( "----- OptimizeAreaGroups Results -----\n" );
	idLib::Printf( "%6i tris in\n", c_in );
	idLib::Printf( "%6i tris after edge removal optimization\n", c_edge );
	idLib::Printf( "%6i tris after final t junction fixing\n", c_tjunc2 );
}
