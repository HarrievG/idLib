...
void idAASReach::FlagReachableAreas( idAASFileLocal* file )
{
	...
	for( i = 1; i < file->areas.Num(); i++ ) {
		if( ( file->areas[i].flags & ( AREA_FLOOR | AREA_LADDER ) ) || ( file->areas[i].contents & AREACONTENTS_WATER ) ) {
...
bool idAASReach::Build( const idMapFile* mapFile, idAASFileLocal* file )
{
	...
	idLib::RogmapPacifierInfo( "[Reachability]\n" );
	...
	idLib::RogmapPacifierCompileProgressTotal( file->areas.Num() - 1 );

	lastPercent = -1;
	for( i = 1; i < file->areas.Num(); i++, idLib::RogmapPacifierCompileProgressIncrement( 1 ) ) {
		if( !( file->areas[i].flags & AREA_REACHABLE_WALK ) ) {
			continue;
		}
...
#if !defined( DMAP )
		int percent = 100 * i / file->areas.Num();
