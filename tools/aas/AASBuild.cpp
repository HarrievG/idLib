...
bool idAASBuild::Build( const idStr& fileName, const idAASSettings* settings )
{
	...
	idLib::RogmapPacifierFilename( name, "Compiling AAS" );
	name.SetFileExtension( "map" );
	...
	if( !bsp.RemoveOutside( mapFile, AREACONTENTS_SOLID, entityClassNames ) ) {
		bsp.LeakFile( name );
		delete mapFile;
		idLib::Warning( "%s has no outside", name.c_str() );
		return false;
	}
	...
	idLib::Printf( "%6d seconds to create AAS\n", ( SDL_GetTicksNS() * 1000 - startTime ) / 1000 );
	idLib::RogmapPacifierInfo( "%6d seconds to create AAS\n", ( SDL_GetTicksNS() * 1000 - startTime ) / 1000 );
...
void RunAAS_f( const idCmdArgs& args )
{
	...
	idLib::ClearWarnings( "compiling AAS" );
...
void RunReach_f( const idCmdArgs& args )
{
	...
	idLib::ClearWarnings( "calculating AAS reachability" );
