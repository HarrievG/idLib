...
//	if( !wPath ) {
//		idLib::FatalError( "Failed to convert path to wide string: '%s'", path );
//		return;
//	}
//
...
//			idLib::FatalError( "CreateDirectoryW failed (error %lu)", err );
//		}
//	}
//
//	free( wPath );
...
//	if( success == 0 ) {
//		DWORD err = GetLastError();
//		idLib::FatalError( "RemoveDirectoryW failed (error %lu)", err );
//	}
//
//	free( wPath );
//	return success != 0;
...
//	if( !wDir ) {
//		idLib::FatalError( "Failed to convert path to wide string: '%s'", directory );
//		return -1;
//	}
//
//	idStr extPattern = "*";
