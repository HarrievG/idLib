...
int Sys_ListFiles( const char* directory, const char* extension, idStrList& list )
{
	...
	if( ( fdir = opendir( directory ) ) == NULL ) {
		if( debug ) {
			idLib::Printf( "Sys_ListFiles: opendir %s failed\n", directory );
		}
		return -1;
	}
	...
	// if( entry == NULL )
	//{
	//	idLib::Warning( "Sys_ListFiles: Mem_Alloc for entry failed!" );
	//	closedir( fdir );
	//	return 0;
	// }
	...
	if( debug ) {
		idLib::Printf( "Sys_ListFiles: %d entries in %s\n", list.Num(), directory );
	}
...
double Sys_ClockTicksPerSecond()
{
	...
	init = true;
	idLib::Printf( "measured CPU frequency: %g MHz\n", ret / 1000000.0 );
	return ret;
}
...
const char*	 Sys_DefaultBasePath()
{
	...
	if( basepath.Length() ) {
		...
		if( stat( testbase.c_str(), &st ) != -1 && S_ISDIR( st.st_mode ) ) {
			...
			idLib::Printf( "no '%s' directory in exe path %s, skipping\n", BASE_GAMEDIR, basepath.c_str() );
		}
	}
	if( basepath != Posix_Cwd() ) {
		...
		if( stat( testbase.c_str(), &st ) != -1 && S_ISDIR( st.st_mode ) ) {
			...
			idLib::Printf( "no '%s' directory in cwd path %s, skipping\n", BASE_GAMEDIR, basepath.c_str() );
		}
	}
	if( exepath.Length() ) {
		...
		if( stat( testbase.c_str(), &st ) != -1 && S_ISDIR( st.st_mode ) ) {
			...
			idLib::Printf( "no '%s' directory in macOS app bundle resources path %s, skipping\n", BASE_GAMEDIR, basepath.c_str() );
		}
#endif
		...
		if( stat( testbase.c_str(), &st ) != -1 && S_ISDIR( st.st_mode ) ) {
			...
			idLib::Printf( "no '%s' directory in build path %s, skipping\n", BASE_GAMEDIR, basepath.c_str() );
		}
	}
	idLib::Printf( "WARNING: using hardcoded default base path %s\n", DEFAULT_BASEPATH );
...
int main( int argc, char** argv )
{
	...
	while( true )
	{
		...
		idLib::UpdateScreen( captureToImage );
	}
	#endif
